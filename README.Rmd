...
time.point = str_c('hpi.', time.point)) -> cleaned_data
```
...
cleaned_data %>% spread(time.point, value) -> expr_data

expr_data %>% arrange(desc(hpi.6)) %>% select(plant) %>% slice(1)
```
...
```{r}
# translate to dplyr code
 expr_data %>% group_by('plant', 'fungus') %>% arrange(-hpi.18) %>% select(plant, fungus) %>% slice(1)
```
...
library(valr)
library(dplyr)
expr_data %>% mutate(expr_var = hpi.24 - hpi.6) -> expr_data
expr_data %>% group_by(gene.name, plant, fungus) %>% summarise(.mean = mean(expr_var), .var = var(expr_var)) %>% group_by(plant, fungus) %>% arrange(desc(.var), desc(.mean)) %>% slice(1:3) -> summary_data 
...
```{r problem_5}
library(ggplot2)
semi_join(expr_data, summary_data) -> joined_data
ggplot(joined_data, aes( x = expr_var, y = hpi.6, color = gene.name)) + geom_point() + geom_smooth(method = "lm", se = FALSE) 
ggplot(joined_data, aes( x = expr_var, y = hpi.12, color = gene.name)) + geom_point() + geom_smooth(method = "lm", se = FALSE) 
ggplot(joined_data, aes( x = expr_var, y = hpi.18, color = gene.name)) + geom_point() + geom_smooth(method = "lm", se = FALSE) 
ggplot(joined_data, aes( x = expr_var, y = hpi.24, color = gene.name)) + geom_point() + geom_smooth(method = "lm", se = FALSE) 
```
